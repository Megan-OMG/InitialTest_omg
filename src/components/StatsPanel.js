...
const StatsPanel = ({ stats, onMine, canMine }) => {
  ...
  return (
    <div className="stats-panel">
      ...
      <button className="mine-button" onClick={onMine} disabled={!canMine}>
        ⛏️ Mine Block
      </button>
      {!canMine && (
        <p className="mine-hint">Create or select a wallet to start mining.</p>
      )}
    </div>
  );
};
